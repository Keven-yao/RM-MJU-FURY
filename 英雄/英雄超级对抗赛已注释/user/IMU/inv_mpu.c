...
uint8_t i2c_write(uint8_t addr, uint8_t reg, uint8_t len, uint8_t *buf)
{
	MPU_Write_Len(reg, buf, len);
	return 0;
}

uint8_t i2c_read(uint8_t addr, uint8_t reg, uint8_t len, uint8_t *buf)
{
	MPU_Read_Len(reg, buf, len);
	return 0;
}
...
{
	uint8_t res = 0, tryCount = 0;
    while (mpu_init() != 0 && tryCount < 50)
        tryCount++;
	if (tryCount < 50) //初始化MPU6050
	{
