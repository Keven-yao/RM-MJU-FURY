/*
    2021.8.7 
		学习前需要对pid控制有一定了解 pid 的 P调大增快响应速度 D调大增大预判，I消除稳差 参数不是越大越好 P太大会超调 D大就会振荡 I值有累积效应，局部使用
    对3508电机控制需要先看电机文档和官方电机控制的代码  
...
float M3508_PID_KP = 10.0f;
float M3508_PID_KI  = 0.00f;
float M3508_PID_KD  = 0.0f;
float M3508_PID_MAX_OUT = 5000.0f;
float M3508_PID_MAX_IOUT  = 5000.0f;
	

//小陀螺平移力增益
float chassisRotationProportion = 0.75f;
//角度偏移修正 1f 360
#define chassisAngleOffset -0.08f

#define PI 3.14159265358979f
int16_t chassisRotateSpeed = 0;
//设置底盘速度系数 在电脑端的速度就是这个，他设置多少，机器人速度就多少（最好以后改成像遥感一样）
int16_t setW_S_A_Dspeed;

int16_t set_mouse_speed=10; //设置鼠标移动系数0
...
void gears_speed() 
{
	...
	else if(RC_Ctl.key.v & KEY_PRESSED_OFFSET_V)
	{
		gears_flag=1.1;
		Chassis_flag=1.07 ;
		chassisRotationProportion = 0.8;
	}
	else if(RC_Ctl.key.v & KEY_PRESSED_OFFSET_B)
	{
		gears_flag=1.15;
		Chassis_flag=1.09;
		chassisRotationProportion = 0.9;
	}
...
//底盘 转换  读取遥控器底盘设置数据
//小陀螺模式下的前进后退 具体不是我写的 可以找陈昱霖进行询问
void chassisConversion(void)
{
    ...
    chassisRotateSpeed = setM1Speed = setM2Speed = setM3Speed = setM4Speed = 3750*Chassis_flag; //默认的自转
	...
	inputVxData=RC_Ctl.rc.ch[1]+W_S_speed*setW_S_A_Dspeed;
	inputVyData=RC_Ctl.rc.ch[0]+A_D_speed*setW_S_A_Dspeed;
...
//不同模式下pid的值不一样，可以保证机器人在不同等级下控制更加稳定
//这些数据都是根据机器人一步一步测试出来，我们这一届没有找到更好的控制方法，希望下个可以找到更好的控制
void chassis_ctrl(int pid_flag)
{
	switch(pid_flag)
	{
        ...
        M3508_PID_KI  = 0.00f;
        M3508_PID_KD  = 0.0f;
        M3508_PID_MAX_OUT = 5000.0f;
        M3508_PID_MAX_IOUT  = 3000.0f;			
...
static void chassisPidInit(Chassis_PID_t *pid, float maxout, float max_iout, float kp, float ki, float kd)//云台 pid 初始化 
{
    if (pid == NULL)
    {
        return;
...
//该任务会进行15ms，不论进行超时或者已完成，都会在15ms后进行下一个任务，，（任务时间可以进行修改，越少整体反应时间会变快）
